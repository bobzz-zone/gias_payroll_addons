...
def execute(filters=None):
	...
	data_ptkp=frappe.db.sql("""select ssa.employee,ssa.from_date, ssa.income_tax_slab ,tss.from_amount
		...
		where e.branch="{1}" and ssa.from_date <"{0}" and docstatus=1 and ssa.from_date in (select max(from_date) from `tabSalary Structure Assignment` ssa2 where ssa2.employee=ssa.employee and ssa2.from_date <"{0}" and docstatus=1)
